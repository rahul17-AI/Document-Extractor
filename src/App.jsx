  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      if (!contentType.includes('application/json')) {
        ...
        console.error('Non-JSON response from /extract:', textPreview.slice(0, 300));
        throw new Error(
          `Service returned non-JSON response (${response.status}). The extraction server is initializing. Please click "Extract to JSON" again.`
        );
      }
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col">
        ...
        {extractionResult && (
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between border-b border-slate-200 pb-2.5">
              <div className="flex items-center gap-1.5">
                ...
                >
                  ...
                  <span className={`text-[10px] px-1.5 py-0.2 rounded font-mono ${
                    viewMode === 'json' ? 'bg-slate-800 text-blue-200' : 'bg-slate-200 text-slate-700'
                  }`}>
              ...
              <div className="text-xs text-slate-500 font-mono hidden sm:block bg-slate-100 border border-slate-200 px-2.5 py-1 rounded-full text-[11px]">
            ...
            {viewMode === 'json' ? (
              ...
              <>
                ...
                <div className="bg-white border border-slate-200/90 rounded-2xl p-3 sm:p-4 flex flex-col md:flex-row md:items-center justify-between gap-3 shadow-xs">
                  {/* Search Bar */}
                  <div className="relative flex-1 max-w-md">
                    ...
                    <input
                      ...
                      className="w-full pl-8 pr-3 py-1.5 rounded-lg border border-slate-300 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors bg-white"
                    />
                    {searchQuery && (
                      <button
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    <button
                      ...
                      className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold text-slate-700 hover:text-slate-900 bg-white hover:bg-slate-50 border border-slate-200 transition-colors cursor-pointer shadow-2xs"
                      title="Copy full document text"
                    >
                      {allCopied ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5 text-slate-400" />}
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold text-white bg-blue-600 hover:bg-blue-700 transition-colors cursor-pointer shadow-xs active:scale-[0.99]"
                  ...
                  <div className="lg:col-span-8 space-y-4">
                    {filteredSections.length === 0 ? (
                      <div className="bg-white border border-slate-200 rounded-2xl p-8 text-center text-slate-500 text-xs shadow-xs">
                        <Filter className="w-6 h-6 mx-auto text-slate-400 mb-2" />
                        <p className="font-bold text-slate-900">No matching sections found</p>
      ...
      <footer className="mt-auto border-t border-slate-200 py-6 text-center text-xs text-slate-500 bg-white">
        <div className="max-w-5xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-2">
          ...
          <span className="text-slate-600 font-medium">Local PyMuPDF Layout Engine &bull; Customer Workspace</span>
        </div>
      </footer>
    </div>
  );
